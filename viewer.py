...
class Viewer(object):
    ...
    def view(self):
        pangolin.CreateWindowAndBind('LiDAR pose-graph SLAM', 1024, 768)
        ...
        # checkbox
        m_follow_camera = pangolin.VarBool('menu.Follow Camera', value=False, toggle=True)
        ...
        proj = pangolin.ProjectionMatrix(w=1024, h=768, fu=viewpoint_f, fv=viewpoint_f, u0=512, v0=389, zNear=0.1, zFar=300)
        look_view = pangolin.ModelViewLookAt(ex=viewpoint_x, ey=viewpoint_y, ez=viewpoint_z, lx=0, ly=0, lz=0, ux=0, uy=1, uz=-1)
